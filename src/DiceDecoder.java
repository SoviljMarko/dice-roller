...
//    }
    ...
    public static int diceType(String input) {
        return Integer.parseInt(input.toLowerCase().split("d")[1]);
    }

    public static boolean syntaxCorrectness(String input) {
        return input.toLowerCase().contains("d");
    }

    public static int numberOfDices(String input) {
        int n = Integer.parseInt(input.toLowerCase().split("d")[0]);
        return numberOfDicesLimit(n) ? n : -1;
    }
